name: Build Android APK - saleh_jarghoun_1301215038
  ...
  build:
    ...
    steps:
      ...
      - name: Create project directory structure
        run: |
          mkdir -p app/src/main/java/com/saleh/exam2
      ...
      - name: Write settings.gradle
        run: |
          ...
          rootProject.name = "saleh_jarghoun_1301215038"
          include ':app'
          EOF
      ...
      - name: Write app/build.gradle
        run: |
          ...
          android {
              namespace 'com.saleh.exam2'
              compileSdk 34
              defaultConfig {
                  applicationId "com.saleh.exam2"
                  minSdk 24
                  targetSdk 34
                  versionCode 1
                  versionName "1.0"
      ...
      - name: Write proguard-rules.pro
        run: echo "# Add project specific ProGuard rules here." > app/proguard-rules.pro
      ...
      - name: Download gradle wrapper jar
        run: |
          ...
          echo "Wrapper size: $(wc -c < gradle/wrapper/gradle-wrapper.jar) bytes"
      ...
      - name: Write MainActivity.java
        run: |
          cat > app/src/main/java/com/saleh/exam2/MainActivity.java << 'EOF'
      ...
      - name: Write ModeActivity.java
        run: |
          cat > app/src/main/java/com/saleh/exam2/ModeActivity.java << 'EOF'
          package com.saleh.exam2;
          import android.os.Bundle;
      ...
      - name: Write TabPagerAdapter.java
        run: |
          cat > app/src/main/java/com/saleh/exam2/TabPagerAdapter.java << 'EOF'
          package com.saleh.exam2;
      ...
      - name: Write TabFragment.java
        run: |
          cat > app/src/main/java/com/saleh/exam2/TabFragment.java << 'EOF'
          package com.saleh.exam2;
          import android.os.Bundle;
          ...
          public class TabFragment extends Fragment {
              ...
              public static TabFragment newInstance(int n) {
                  TabFragment f = new TabFragment();
                  Bundle args = new Bundle();
                  args.putInt(ARG_TAB, n);
                  f.setArguments(args);
                  return f;
              }
              @Nullable @Override
      ...
      - name: Write activity_mode.xml
        run: |
          cat > app/src/main/res/layout/activity_mode.xml << 'EOF'
          <?xml version="1.0" encoding="utf-8"?>
          <LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
              android:layout_width="match_parent"
              android:layout_height="match_parent"
      ...
      - name: Write fragment_tab.xml
        run: |
          cat > app/src/main/res/layout/fragment_tab.xml << 'EOF'
          <?xml version="1.0" encoding="utf-8"?>
          <LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
              android:layout_width="match_parent"
              android:layout_height="match_parent"
      ...
      - name: Write menu_mode.xml
        run: |
          cat > app/src/main/res/menu/menu_mode.xml << 'EOF'
          <?xml version="1.0" encoding="utf-8"?>
          <menu xmlns:android="http://schemas.android.com/apk/res/android">
              <item android:id="@+id/mode_light"  android:title="Light"  />
              <item android:id="@+id/mode_night"  android:title="Night"  />
              <item android:id="@+id/mode_system" android:title="System" />
          </menu>
          EOF
      ...
      - name: Write strings.xml
        run: |
          ...
          <?xml version="1.0" encoding="utf-8"?>
          <resources>
              <string name="app_name">saleh_jarghoun_1301215038</string>
          </resources>
          EOF
      ...
      - name: Write themes.xml
        run: |
          ...
          <?xml version="1.0" encoding="utf-8"?>
          <resources>
              <style name="Theme.Exam2" parent="Theme.MaterialComponents.DayNight.DarkActionBar">
      ...
      - name: Generate PNG launcher icons
        run: |
          python3 - << 'PYEOF'
          import struct, zlib
          ...
          for dpi, sz in [('mdpi',48),('hdpi',72),('xhdpi',96),('xxhdpi',144),('xxxhdpi',192)]:
              png = make_png(sz, 98, 0, 238)
              for name in ('ic_launcher','ic_launcher_round'):
                  open(f'{base}/mipmap-{dpi}/{name}.png','wb').write(png)
          print("Icons OK")
          PYEOF
      ...
      - name: Show built APKs
        run: find app/build/outputs/apk -name "*.apk" | xargs ls -lh

      - name: Upload Debug APK
        uses: actions/upload-artifact@v4
        with:
          name: saleh_jarghoun_1301215038-debug
      ...
      - name: Upload Release APK
        uses: actions/upload-artifact@v4
        with:
          name: saleh_jarghoun_1301215038-release
